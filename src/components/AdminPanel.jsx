...
export default function AdminPanel({ orders, setOrders }) {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-8">Panel de Administración</h2>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="p-6 space-y-4">
                {orders.length === 0 ? (
                    ...
                    orders.map(order => (
                        <div key={order.id} className={`flex items-center justify-between p-4 bg-${buttonColor}-50 rounded-lg`}>
                            <div>
                            ...
                            <button
                                ...
                                className={`px-4 py-2 bg-${buttonColor}-600 text-white rounded-lg hover:bg-${buttonColor}-700 transition-colors`}
                            >
                                {buttonText}
                            </button>
                        </div>
